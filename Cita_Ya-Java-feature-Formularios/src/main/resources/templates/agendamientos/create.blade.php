    ...
    <form action="{{ route('agendamientos.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="cita" class="form-select" required>
                ...
                <option value="Consulta general" {{ old('cita') == 'Consulta general' ? 'selected' : '' }}>Consulta General</option>
                <option value="Ortopedia" {{ old('cita') == 'Ortopedia' ? 'selected' : '' }}>Ortopedia</option>
                <option value="Psicologia" {{ old('cita') == 'Psicologia' ? 'selected' : '' }}>Psicología</option>
                <option value="Cardiologia" {{ old('cita') == 'Cardiologia' ? 'selected' : '' }}>Cardiología</option>
                <option value="Pediatria" {{ old('cita') == 'Pediatria' ? 'selected' : '' }}>Pediatría</option>
                <option value="Dermatología" {{ old('cita') == 'Dermatología' ? 'selected' : '' }}>Dermatología</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="fecha" class="form-control" value="{{ old('fecha') }}" required min="{{ date('Y-m-d') }}">
        </div>
